...
const StarRating = ({ rating, onRatingChange }: { rating: number, onRatingChange: (value: number) => void }) => {
  return (
    <ReactStars
      count={5}
      value={rating}
    ...
    // <div className="min-h-screen flex items-center justify-center">
     <>
        ...
        <p className="text-gray-600 mb-6">
          Please select which of the following you have used in the past and rate how well they worked for you. (out of 5 stars) <span className="text-red-500">*</span>
        </p>
              ...
              <Field name={`ratings.${service.name}`}>
                {() => (
                  <StarRating
                    ...
                    onRatingChange={(value) => handleRatingChange(`ratings.${service.name}`, value)}
                  />
                )}
              </Field>
            </div>
